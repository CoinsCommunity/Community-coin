...
- **Lattice-based commitments.** Constructions based on the Learning With Errors (LWE) or Short Integer Solution (SIS) problems. Some constructions achieve homomorphism but with larger commitment sizes and more complex proving systems. There is concrete prior art here: the MatRiCT protocol (Esgin, Zhao, Steinfeld, Liu, and Liu, CCS 2019) and MatRiCT+ (IEEE S&P 2022) include a lattice commitment scheme and lattice range proofs designed specifically for post-quantum RingCT, with a working implementation. More recent recursive lattice argument systems, LaBRADOR (CRYPTO 2023) and Greyhound (CRYPTO 2024), provide succinct proofs from Module-SIS that may yield more compact range proofs than earlier constructions. These are the strongest candidates to evaluate. The open question is whether any of them, combined, gives a homomorphic commitment with a logarithmic-size range proof that is both quantum-secure and practical at blockchain scale.

...
The first and hardest is post-quantum sender privacy at full-chain scale. Ring signatures (MLSAG, CLSAG) are obsolete. Monero replaced them in January 2026 with Full-Chain Membership Proofs (FCMP++), which hide a spend among the entire output history instead of a small ring of decoys. But FCMP++ is built on curve trees over an elliptic curve cycle, so it is still discrete-log based and not post-quantum for funds security. The open question is whether a post-quantum analog of a full-chain membership proof exists: lattice or hash based, proving membership in a set of millions of outputs with proof sizes in the low kilobytes. Lattice accumulators with zero-knowledge membership exist (Libert et al.) but run to hundreds of kilobytes at chain scale. The most promising direction is recursive lattice argument systems, LaBRADOR (CRYPTO 2023) and Greyhound (CRYPTO 2024), as a possible analog of curve-cycle recursion, but assembling them into a practical full-chain proof is unsolved. See sections 4.2 and 9.
...
The third is the commitment scheme for confidential transactions. Pedersen commitments are not quantum resistant. Their hiding property depends on discrete log hardness. A replacement needs to be hiding, binding, homomorphic so transaction balances can be verified without revealing amounts, and must support efficient range proofs. This one has the strongest prior art: MatRiCT and MatRiCT+ (Esgin et al., CCS 2019 and IEEE S&P 2022) are implemented post-quantum lattice RingCT protocols with a lattice commitment and range proofs, so this is the most tractable of the three, pending independent review. See sections 4.4 and 9.7.
